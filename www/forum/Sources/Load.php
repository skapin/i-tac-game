...
function testChangePerso(){
  global $cookiename,$db_prefix,$ID_MEMBER;
  if(!isset($_POST['persoId']) ||
     !is_numeric($_POST['persoId']) ||
     empty($_SESSION['com_ID'])){
    return;
  }
  // tentative de changement de compte forum.
  $result = db_query("
		SELECT ID_MEMBER,
ID_GROUP,
passwd,
...
additionalGroups
		...
		WHERE ID_compte = '" .$_SESSION['com_ID']. "'
                AND ID_MEMBER='".$_POST['persoId']."'
		LIMIT 1", __FILE__, __LINE__);

  if($plop=mysql_fetch_array($result)){
    $plop['additionalGroups']=explode(',',$plop['additionalGroups']);
    // Mise en place du cookie
    // Truc original pour l'etat :
    // $cookie_state = (empty($modSettings['localCookies']) ? 0 : 1) | (empty($modSettings['globalCookies']) ? 0 : 2);
    $state=0;
    $data=serialize(array($plop['ID_MEMBER'],
			  sha1($plop['passwd'].$plop['passwordSalt']),
			  time() + 600,
			  $state));
    setcookie($cookiename, $data, time() + 86400,'/');//,$cookie_url[1], $cookie_url[0], 0);
    ...
    if ($plop['ID_GROUP'] == 1 || in_array(1,$plop['additionalGroups'])){
      $_SESSION['admin_time'] = time();
      unset($_SESSION['just_registered']);
    }
    // Update d'infos sur le compte
    db_query('UPDATE smf_members SET 
lastLogin='.time().',
memberIP="'.$_SERVER['REMOTE_ADDR'].'",
memberIP2=""
 WHERE ID_MEMBER='.$plop['ID_MEMBER'],__FILE__,__LINE__);
    //'.$_SERVER['BAN_CHECK_IP'].'
    // On vire l'ancienne entree du guest dans la liste des actifs
    db_query("DELETE FROM smf_log_online
	     WHERE session = 'ip".$_SERVER['REMOTE_ADDR']."'
	     LIMIT 1",__FILE__,__LINE__);
    $_SESSION['log_time'] = 0;
    $ID_MEMBER=$plop['ID_MEMBER'];
  }
}
