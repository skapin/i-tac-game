...
function MessageFolder()
{
	...
	// View Single PM. Check that our parameter is valid.
	if(isset($_GET['p']))
		$context['sl-singlepm'] = (int) $_GET['p'];

	// If we haven't changed settings it is blank. Lets fix it.
	...
	// If We are Viewing Single Post lets get its Tree.
	...
	// We had to Temporally Upgrade our p variable for the Tree. Fix it..
	if(isset($context['sl-singlepm']))
		--$context['sl-singlepm'];
	...
	// View Single PM. If there is no pm to show then we got to quit. But Dont log it.
...
function MessagePost()
{
	...
	else if(isset($_REQUEST['nick']))
	{
		$_REQUEST['nick'] = explode(',', $_REQUEST['nick']);
		foreach ($_REQUEST['nick'] as $key => $uID)
			$_REQUEST['nick'][$key] = (int) $uID;
		$request = db_query("
			SELECT realName
			FROM {$db_prefix}members
			WHERE ID_perso IN (" . implode(', ', $_REQUEST['nick']) . ")
			LIMIT " . count($_REQUEST['nick']), __FILE__, __LINE__);
		while ($row = mysql_fetch_assoc($request))
			$membersTo[] = '&quot;' . $row['realName'] . '&quot;';
		mysql_free_result($request);
		$_REQUEST['to'] = implode(', ', $membersTo);
