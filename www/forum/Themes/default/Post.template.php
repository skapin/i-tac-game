...
'; 
// Quick Upload lays here - by Elmacik
echo '
...
function template_postbox(&$message)
{
	...
	if ($context['show_bbc'])
	{
		...
		if (!empty($modSettings['customBBCode_count']))
		{
			...
			{
				if (!empty($tag) && !empty($modSettings['customBBCode_enable_' . $i]) && !empty($modSettings['customBBCode_button_' . $i]))
				{
					$context['bbc_tags'][1][$tag]['code'] = $tag; 
					...
					if ($tag_type == 1)
						$context['bbc_tags'][1][$tag]['before'] = '[' . $tag . '=]'; 
					else
						$context['bbc_tags'][1][$tag]['before'] = '[' . $tag . ']'; 
					
					if ($tag_type != 2)
						$context['bbc_tags'][1][$tag]['after'] = '[/' . $tag . ']'; 

					if (!empty($modSettings['customBBCode_description_' . $i]))
						$context['bbc_tags'][1][$tag]['description'] = $modSettings['customBBCode_description_' . $i];
					else
						$context['bbc_tags'][1][$tag]['description'] = $tag; 
				}
			}
		}
